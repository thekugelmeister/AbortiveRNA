...
# 1. parses "*.trmout.forward", "*.trmout.reverse", and "*.RBSMatches"
...
for i in range(0,numEntries):
    ...
    # reverse abortSeq
    abortSeq = abortSeq[::-1]
    tranName = lines[(6 * i) + 1].split()[1]
    strand = lines[(6 * i) + 1].split()[5]
    RBSName = lines[(6 * i) + 3].split()[1]
    rOFile.write(tranName + " " + RBSName + " " + strand + " " + bindSeq + " " + abortSeq + " ")
    # write the binding site and abortive sequences to the appropriate files
    b = open('bind.seq', 'w')
    a = open('abort.seq', 'w')
    b.write(bindSeq)
