...
tOFile.write("Operon TranscriptionUnit Terminator Strand BindingSiteSequence faRNASequence faRNALength DeltaGTerminator DeltaGBinding tranLB tranRB termLB termRB\n")
...
for f in [termForward, termReverse]:
    ...
    for i in range(0,numEntries):
        ...
        tranLeft = lines[(5 * i) + 1].split()[3]
        tranRight = lines[(5 * i) + 1].split()[4]
        termName = lines[(5 * i) + 3].split()[1]
        termLeft = lines[(5 * i) + 3].split()[3]
        termRight = lines[(5 * i) + 3].split()[4]
        termDG = lines[(5 * i) + 3].split()[6]
        
        # check for term/promoter overlap; is TSS within boundaries of terminator?
        ...
        if bindSeq == abortSeq:
            ...
            # tOFile.write( str(float(l[106].split()[1]) - float(l[6].split()[1])) + "\n" )
            tOFile.write( str(float(l[4].split()[1]) - float(l[3].split()[1])) + " " )
        tOFile.write(tranLeft + " " + tranRight + " " + termLeft + " " + termRight + "\n")
        # clean up directory
...
rOFile.write("Operon TranscriptionUnit RBS Strand BindingSiteSequence faRNASequence faRNALength DeltaGBinding\n")
for i in range(0,numEntries):
    # get the required values for the given entry
